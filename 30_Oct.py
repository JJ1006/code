...
# Constants
coin_values = np.array([0.01, 0.05, 0.10, 0.25, 0.50, 1.00])  # Possible coin values (face values in dollars)
...
def bisection_method(func, interval, tol=0.00001, max_iter=1000):
    ...
    while abs(b - a) > tol and iter_count < max_iter:
        ...
        print(f"Iteration {iter_count+1}: a = {a:.8f}, b = {b:.8f}, mid = {mid:.8f}, mean_diff = {f_mid:.8f}")
    ...
    print(f"\nFinal tolerance achieved: {final_tolerance:.8f} (Target tolerance: {tol})")
...
print("Finding optimal alpha using bisection method:")
...
for i, prob in enumerate(optimal_probabilities, start=1):
    print(f"\t• Coin {i} (value ${coin_values[i-1]:.2f}): Probability = {prob:.8f}")
print(f"Estimated mean: {estimated_mean:.8f}")
print(f"Loss function value (difference in mean): {loss_value:.8f}")
